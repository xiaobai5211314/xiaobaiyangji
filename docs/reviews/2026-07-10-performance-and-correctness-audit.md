...
6. OCR 晚间导入收益重复计算：`FundController` 的 `estimate_today` 分支中，有新鲜 OCR 快照时仍把盘中估算收益叠加到 OCR 当前金额上。晚间 OCR 截图的当前金额已是平台当天最终值（含当日收益），叠加盘中估算收益导致收益被计算两遍。修复后有新鲜 OCR 时 `rawHoldAmount` 保持 OCR 当前金额，盘中估算收益仅用于"今日盘中估算"临时展示，不进入账户总金额。
