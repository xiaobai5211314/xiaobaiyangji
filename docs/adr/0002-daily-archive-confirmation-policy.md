...
7. **官方净值滚动金额**：缺少蚂蚁 OCR 确认时，`official-nav-pending` 档案的金额按上一有效金额加官方净值确认收益滚动；内部持仓金额和结算收益底稿可保留 4 位小数，写入 `DailyArchive` 时再标准化为 2 位；不得用 `份额 × 今日净值` 直接覆盖，以避免自动反推份额导致的分位差。
