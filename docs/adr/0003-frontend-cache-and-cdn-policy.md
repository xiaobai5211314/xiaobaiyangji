...
- 部署后自动清除 CDN 对应路径缓存；刷新 API 返回失败时必须使工作流失败，禁止忽略错误后继续标记发布成功。
- 刷新完成后，工作流必须访问不带查询参数的 `https://guzhicdn.21212121.xyz/index.html`，并比较响应正文 SHA-256 与本次 `wwwroot/index.html`。在有限重试时间内仍不一致时必须使工作流失败。
- 后端部署会复制 `wwwroot/` 到直连站点，但不会代替又拍云上传；因此只能以 CDN 正文校验作为 CDN 已更新的证据。
...
- WebApp 各视口保留固定底部导航，并使用 `border-box`、安全区和页面底部留白保证内容可滚动到导航上方；不得改为遮挡首屏内容的顶部粘性导航。
...
- 不得仅靠全局 `overflow` 裁切掩盖控件越界；基金名称等明确的信息密集列表可以使用省略号，但金额、状态、操作按钮和弹窗标题必须保持完整可访问。
...
- 直连站点和 CDN 的 `index.html` 必须来自同一 Git 提交；发布结果以 CDN 正文 SHA-256 校验通过为准，不能只以 GitHub Actions 的上传步骤成功为准。
