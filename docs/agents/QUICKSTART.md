...
```
/diagnose 晚上 11 点 OCR 导入后，总持仓金额比蚂蚁 App 显示的高。只分析原因，不要修改代码。
```

```
/diagnose 周末或交易日 09:30 前，今日收益仍显示上一交易日数值。对照 naturalDate、effectiveDate、marketStatus 和 DailyArchive，只分析原因，不要修改代码。
```
...
```
/tdd 同一自然日 OCR 当前金额不能再次叠加当日收益，跨过 00:00 后今日收益必须归零。覆盖周末、节假日和工作日 09:29/09:30 边界。
```
...
8. 检查推文“原文链接”和回复“回复原文”按钮能打开 X 原文；内嵌 WebView 下要确认同页跳转兜底是否生效。
9. 用 `git diff --name-only` 确认未修改收益计算、OCR、`DailyArchive`、盈亏日历、首页 summary 和登录逻辑。
