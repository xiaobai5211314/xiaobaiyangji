...
namespace 估值助手.Services
{
    ...
    public sealed class EastmoneyStockQuoteService : IStockQuoteService
    {
        ...
        {
            code = NormalizeCode(code);
            // 东方财富 secid：1=沪市，0=深市。北交所编码历史上存在调整，接口兼容性待核实；先按 0 尝试，失败时前端展示空数据。
        ...
        public async Task<IReadOnlyList<StockSearchDto>> SearchAsync(string keyword, CancellationToken cancellationToken = default)
        {
            ...
            if (IsStockCode(NormalizeCode(keyword)))
            {
                var q = await GetQuoteAsync(keyword, cancellationToken);
                ...
                && arr.ValueKind == JsonValueKind.Array)
            {
                foreach (var item in arr.EnumerateArray())
                {
                    ...
                    result.Add(new StockSearchDto(
                        ...
                        GetString(item, "SecurityTypeName", "stock")));
                }
            }
        ...
        private static bool IsStockCode(string code) => Regex.IsMatch(NormalizeCode(code), "^[0-9]{6}$");

        private static string NormalizeCode(string code)
        {
            var digits = Regex.Replace(code ?? string.Empty, "\\D", "");
