...
- 响应式布局：WebApp 手机端使用带安全区的底部导航，769px 及以上改为占据正常布局空间的顶部粘性导航；金额、状态、弹窗和工具栏不得依赖隐藏溢出来规避遮挡。小程序页面为固定底栏预留滚动空间，页头、日历工具栏和操作区允许安全换行。
...
- 资产详情页 OCR 到份额和成本价时，系统优先按平台字段校准份额、成本金额和 `RealizedProfit`；常规 OCR 截图缺少份额或成本价时，系统可自动推导辅助字段。派生份额和成本不得覆盖同日蚂蚁 OCR 的金额、昨日收益或持有收益。
