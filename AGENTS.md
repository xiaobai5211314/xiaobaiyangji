...
- “当前展示 OCR”只指 `OcrSnapshotDate` 与北京时间自然日相同的截图时点快照；盘中估值不得重复叠加。截图后正式净值更新且可靠份额证明 OCR/滚动金额存在超过 `0.01` 元的基数异常时，允许按官方金额自愈；跨过 00:00 后旧快照退出当前展示优先级。
- 当前金额必须与确认持仓金额拆开：`平台当前金额 = 已确认持仓金额 + 买入待确认金额`。
- 待确认买入的本金和成本只能各隔离一次：持有收益与 `DailyArchive.TotalProfit` 使用 `确认持仓金额 - 确认成本 + RealizedProfit + PlatformHoldingAdjustment`；仅未确认的 `purchase_amount` 成本扣除有效待确认买入，平台/OCR 已确认成本不得重复扣除。
- `PendingConfirmDate` / `FirstProfitDate` 只控制预计收益参与时间，不代表平台已确认。跨过 00:00 或预计日期不得隐藏/清除 pending；只有正式净值结转拿到份额、OCR 明确归零、取消或其他可核验结果才能结束待确认状态。
- 待确认赎回必须两阶段结算：预计确认日到达且取得原交易日正式净值后，只结转剩余份额、金额和成本；平台实际到账确认后只补记 `RealizedProfit`。两阶段均须幂等，严禁到账确认时再次扣减持仓。
...
- 自然日与有效交易日必须分开：周末、节假日和交易日 09:30 前可以沿用上一交易日金额与累计收益，但当前自然日的今日收益、今日收益率和单基金今日涨跌幅必须为 0；不得把上一交易日档案重新标成 `official_today`。
...
- P3 份额或成本价缺失时按 ADR 自动推导或留空，不要求用户手工补查；资产详情页给出这些字段时应优先使用平台值。

### 前端主题

- 正式 WebApp 和微信小程序只保留两个主题：默认深色“曜石流光”和浅色“雾光银蓝”。旧主题值只允许在读取本地历史设置时映射到这两个主题，不得恢复成额外主题选项。
- 浅色主题正文、状态、金额和控件文字必须使用可读的深色文本；白字只用于系统蓝或其他足够深的激活背景。前端主题修改后必须做手机宽度渲染和文字对比度检查。
...
- 本仓库当前主分支约定为 `master`；ahead/behind、工作树和远端状态会变化，任何 Git 操作前必须重新运行 `git status --short --branch` 核对。
